...
func TestProcessorStreamableSessionContinuity(t *testing.T) {
	t.Parallel()
	...
	initID, err := jsonrpc.MakeID("initialize")
	require.NoError(t, err)

	conn.responseHeaders = http.Header{
		mcpclient.HeaderSessionID: []string{"session-abc"},
	}
	conn.response = &jsonrpc.Response{
		ID:     initID,
		Result: json.RawMessage(`{"ok":true}`),
	}
	conn.statusCode = http.StatusOK

	initCmd := &fakePolledCommand{
		id:         types.RequestID("init-request"),
		message:    &jsonrpc.Request{ID: initID, Method: "initialize"},
		enqueuedAt: time.Now(),
		polledAt:   time.Now(),
		shardToken: "shard-init",
	}

	require.NoError(t, processor.Process(context.Background(), initCmd))
	_ = responder.waitForResponse(t)
	...
	callCmd := &fakePolledCommand{
		id:         types.RequestID("call-request"),
		message:    &jsonrpc.Request{ID: callID, Method: "tools/list"},
		enqueuedAt: time.Now(),
		polledAt:   time.Now(),
		shardToken: "shard-call",
	}

	require.NoError(t, processor.Process(context.Background(), callCmd))
	_ = responder.waitForResponse(t)

	require.NotNil(t, conn.writeHeaders)
	require.Equal(t, "session-abc", conn.writeHeaders.Get(mcpclient.HeaderSessionID))
}
